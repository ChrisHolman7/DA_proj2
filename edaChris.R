...
sub4$amt_lifetime_giving_log = log(sub4$amt_lifetime_giving+1)

model1 = lm(amt_lifetime_giving_log ~ ., data=sub4[,c(14:30,36:49,58)])
...
cor(na.omit(sub4[,c(14:30,36:49,57,58)]))


#NO PROGRESS TO REMOVE MORE VARIABLES

#CHECKING DIST OF INVESTMENT VARS
par(mfrow=c(4,2))

logTrans = c(23,24,27)
for (i in 23:30){
  if (i %in% logTrans){
    hist(log(sub4[,i]), main = paste(colnames(sub4)[i], 'log'))
  } else{
    hist((sub4[,i]), main =  colnames(sub4)[i])
  }
...
par(mfrow=c(5,3))
logTrans2 = c(42,45,46,49)
for (i in 36:49){
  if (i %in% logTrans2){
    hist(log(sub4[,i]), main = paste(i ,colnames(sub4)[i], 'log'))
  } else{
    hist((sub4[,i]), main =  paste(i , colnames(sub4)[i]))
  }
  
}

bigcor = cor(na.omit(sub4[,c(36:49)]))
ggcorrplot(bigcor, hc.order=TRUE,type='lower',lab=TRUE)
